...
@keyframes micPulseGreen { 
    ...
    70% { transform: scale(1.15); box-shadow: 0 0 0 20px rgba(16, 185, 129, 0); } 
    100% { transform: scale(1); box-shadow: 0 0 0 0 rgba(16, 185, 129, 0); } 
}
/* 对讲机按钮样式 */
.big-mic-btn {
    display: flex; align-items: center; justify-content: center;
    width: 75px; height: 75px; border-radius: 50%; margin: 10px auto;
    background: #ef4444; color: white; font-size: 2.5rem;
    border: 4px solid #fca5a5; box-shadow: 0 8px 15px rgba(239,68,68,0.4);
    cursor: pointer; transition: all 0.2s cubic-bezier(0.34, 1.56, 0.64, 1);
}
.big-mic-btn:active { transform: scale(0.85); box-shadow: 0 2px 5px rgba(239,68,68,0.4); }
/* 正在录音时的绿灯状态 */
.big-mic-btn.listening {
    background: #10b981; border-color: #6ee7b7;
    box-shadow: 0 8px 15px rgba(16,185,129,0.4);
    animation: micPulseGreen 1.5s infinite;
}
`;
...
function loadUser(uid) { currentUid = uid; localStorage.setItem('mw_cur', uid); const ws = localStorage.getItem(wKey(uid)); if (ws) { words = JSON.parse(ws); words.forEach(w => { if (!w.bookId) w.bookId='default'; }); saveW(); } else { words = SEED.map((s,i) => ({id:'seed_'+i, english:s.english, chinese:s.chinese, emoji:s.emoji, level:0, nextReview:Date.now(), bookId:'default'})); saveW(); } const ps = localStorage.getItem(pKey(uid)); player = ps ? JSON.parse(ps) : {}; if(player.dailyNew===undefined) player.dailyNew = 10; if(player.dailyRev===undefined) player.dailyRev = 30; if(player.streak===undefined) player.streak = 0; if(player.coins===undefined) player.coins = 0; if(player.fullness===undefined) player.fullness = 0; if(player.totalFed===undefined) player.totalFed = 0; if(player.quizMode===undefined) player.quizMode = 'en2zh'; currentBookId = localStorage.getItem('mw_cbook_'+uid) || 'default'; }
function buildQueue(forceRebuild = false) { const now = Date.now(); const today = new Date().toDateString(); if (player.lastDate !== today || forceRebuild) { if (player.lastDate !== today) { player.todayNew = 0; player.todayRev = 0; player.fullness = 0; } player.lastDate = today; saveP(); } let candNew = []; let candRev = []; words.forEach((w, i) => { if (w.bookId === currentBookId && (w.nextReview || 0) <= now) { if (w.level === 0) candNew.push(i); else candRev.push(i); } }); candNew.sort(() => Math.random() - 0.5); candRev.sort(() => Math.random() - 0.5); return [...candNew.slice(0, Math.max(0, (player.dailyNew || 10) - (player.todayNew || 0))), ...candRev.slice(0, Math.max(0, (player.dailyRev || 30) - (player.todayRev || 0)))].sort(() => Math.random() - 0.5); }
...
async function autoSpeakZh(text) { return new Promise((r) => { const url = `https://dict.youdao.com/dictvoice?audio=${encodeURIComponent(text)}&le=zh`; try { const a = new Audio(url); a.onended=r; a.onerror=r; a.play().catch(r); } catch(err) { r(); } setTimeout(r, 1500); }); }
...
function manualStartListening(e, wordInfo) {
    e.stopPropagation();
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) { Dialog.alert('浏览器不支持语音，请用平板/手机的 Chrome'); return; }
    ...
    recognition.interimResults = true; // 开启实时出字
    ...
    const hintEl = document.getElementById('hintInfo');
    if(hintEl) hintEl.innerHTML = '<span class="text-emerald-700">正在听，请大胆说...</span>';
    ...
    recognition.onresult = function(event) {
        ...
        let rawText = finalTranscript || interimTranscript;
        if(!rawText) return;
    ...
    recognition.onerror = function() { 
        if(btn) btn.classList.remove('listening');
        if(hintEl) hintEl.innerHTML = '<span class="text-rose-500 font-bold">没有听清声音哦，再点一次</span>'; 
    };
...
function showCard() {
  if (recognition) { recognition.abort(); } // 换卡强制切断旧录音
  ...
  // 👉 核心UI渲染：如果开了对讲机，则不显示那排字，而是展现对讲按钮
  if(isMicEnabled) {
      hintContainer.innerHTML = `
        <div id="bigMicBtn" class="big-mic-btn" title="点击开始对讲录音">🎙️</div>
        <div id="hintInfo" class="text-slate-400 font-bold mt-2 text-sm pt-2 border-t border-slate-100/50">👆 点击麦克风说答案，或点屏幕翻面</div>
      `;
      // 绑定点击事件，把当前的词信息传进去
      setTimeout(() => {
          const btn = document.getElementById('bigMicBtn');
          if(btn) btn.onclick = (e) => manualStartListening(e, w);
      }, 50);
  } else {
      hintContainer.innerHTML = '<span class="text-slate-400 font-bold text-sm">👆 点击卡片翻看答案</span>';
  }
  ...
  renderDots(); renderStats(); const c=document.getElementById('card'); c.classList.remove('cin'); void c.offsetWidth; c.classList.add('cin');
}
