/* ═══════════════════════════════════════════════
   1. 🧨 强力破冰针：粉碎 PWA 顽固离线缓存
   ═══════════════════════════════════════════════ */
if ('serviceWorker' in navigator) {
    navigator.serviceWorker.getRegistrations().then(function(registrations) {
        for(let registration of registrations) {
            registration.unregister(); // 强行解除华为 Pad 上的锁死版缓存
        }
    });
}
localStorage.removeItem('mw_mic'); // 清理过期的对讲机遗留标志
...
function loadUser(uid) { 
    ...
    if (ws) { words = JSON.parse(ws); words.forEach(w => { if (!w.bookId) w.bookId='default'; }); saveW(); } else { words = []; saveW(); } 
...
function showCard() {
  ...
  hintContainer.innerHTML = '<span class="text-slate-400 font-bold text-sm tracking-wide">👆 点击卡片整体翻开</span>';
...
async function importWords() { 
    ...
    Dialog.alert('🔍 正在全网下载高清网图，请耐心等待勿关闭网页...', 'alert'); 
    let ok=0; const p = []; 
    raw.split('\n').forEach(l => { const pp = l.trim().split(/[,，]/); if(pp.length>=2) p.push({ en:pp[0].trim(), zh:pp[1].trim(), emo:pp[2]?.trim() }); }); 
    if (!p.length) return; 
    for (let i = 0; i < p.length; i++) { if (!p[i].emo) p[i].emo = await fetchWikiImage(p[i].en) || autoEmoji(p[i].en); } 
    USERS.forEach(u => { 
    ...
    document.getElementById('ta').value = ''; Dialog.alert(`✅ 棒！成功导入 ${ok} 个新词，配图已保存！`); refreshList(); queue = buildQueue(); qi = 0; showCard(); renderStats(); 
}
...
async function deleteBook() { if(books.length<=1){await Dialog.alert('至少留1个系统词库哦');return;} if(currentBookId === 'default') {await Dialog.alert('这个词库不可删！');return;} if(!(await Dialog.confirm('⚠️ 警告：删除此库会清空里面所有的单词？'))) return; USERS.forEach(u=>{ const ws=localStorage.getItem(wKey(u.uid)); if(ws){ const arr=JSON.parse(ws).filter(w=>w.bookId!==currentBookId); localStorage.setItem(wKey(u.uid),JSON.stringify(arr)); if(u.uid===currentUid)words=arr; } }); books=books.filter(b=>b.id!==currentBookId); saveBooks(); currentBookId=books[0].id; localStorage.setItem('mw_cbook_',currentBookId); queue=buildQueue(); qi=0; renderBookTabs(); refreshList(); renderStats(); showCard(); }

function petTap() { const st = getPetStage(); const msgs = ['哎呀，别戳我！', '金币能让小怪兽长大！', '快去买零食喂我！']; if(player.ownsCrown) msgs.push('膜拜吧！理财带师💰'); if(st.level >= 4) msgs.push('大鲨鱼饿啦！🦈'); const el = document.createElement('div'); el.textContent = msgs[Math.floor(Math.random() * msgs.length)]; el.style.cssText = 'position:fixed; top:20%; left:50%; transform:translateX(-50%); background:white; color:#333; font-weight:bold; padding:10px 20px; border-radius:16px; box-shadow:0 10px 20px rgba(0,0,0,0.1); z-index:9999; animation:flyup 1.8s forwards; font-size:0.9rem; pointer-events:none;'; document.body.appendChild(el); setTimeout(()=>el.remove(), 1800); }
